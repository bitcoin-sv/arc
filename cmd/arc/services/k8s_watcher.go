...
func StartK8sWatcher(logger *slog.Logger, arcConfig *config.ArcConfig) (func(), error) {
	...
	mtmConn, err := metamorph.DialGRPC(arcConfig.Metamorph.DialAddr, arcConfig.PrometheusEndpoint, arcConfig.GrpcMessageSize)
	if err != nil {
	...
	blocktxConn, err := blocktx.DialGRPC(arcConfig.Blocktx.DialAddr, arcConfig.PrometheusEndpoint, arcConfig.GrpcMessageSize)
	if err != nil {

...
func (m *SendManager) singleSend() {
	...
	m.logger.Info("=== Sending single callback", slog.String("url", m.url), slog.String("token", callbacks[0].Token), slog.String("txid", callbacks[0].TxID), slog.String("status", callbacks[0].TxStatus))

...
func WithSubscribedInterestPolicy(consumerName string, topics []string, cleanUpConsumer bool) func(handler *Client) error {
	return func(c *Client) error {
		for _, topic := range topics {
			...
			cons, err := c.getConsumer(stream, fmt.Sprintf("%s-%s-cons", consumerName, topic))
			if err != nil {
				return errors.Join(ErrFailedToGetConsumer, err)
			}

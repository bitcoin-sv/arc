...
func TestReRequestPending(t *testing.T) {
	...
	t.Run("re-request pending", func(t *testing.T) {
		...
		err = d.Get(&requestedAt1, "SELECT requested_at FROM metamorph.transactions WHERE hash = $1", chainHash1[:])
		assert.NoError(t, err)
		...
		err = d.Get(&requestedAt2, "SELECT requested_at FROM metamorph.transactions WHERE hash = $1", chainHash2[:])
		assert.NoError(t, err)
		assert.True(t, expectedRequestedAt2.Equal(requestedAt2))

		var requestedAt3 time.Time
		expectedRequestedAt3 := now
		err = d.Get(&requestedAt3, "SELECT requested_at FROM metamorph.transactions WHERE hash = $1", chainHash3[:])
		assert.NoError(t, err)

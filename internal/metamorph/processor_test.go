...
// 			pm := better_p2p.NewBetterPeerManager(slog.Default(), wire.TestNet)
// 			peerMq := &better_p2p_mocks.PeerIMock{}
// 			peerMq.WriteMsgFunc = func(msg wire.Message) {
// 				txmsg := msg.(*wire.MsgInv)
// 				require.Equal(t, 1, len(txmsg.InvList), "too many inv in wire message")

// 				iv := txmsg.InvList[0]
// 				require.True(t, testdata.TX1Hash.IsEqual(&iv.Hash), "unexpected INV hash")
// 			}
// 			peerMq.NetworkFunc = func() wire.BitcoinNet { return wire.TestNet }

// 			pm.AddPeer(peerMq)

// 			herald := better_p2p.NewHerald(pm)
...
// 			sut, err := metamorph.NewProcessor(s, cStore, herald, nil, metamorph.WithMessageQueueClient(publisher))
// 			require.NoError(t, err)
...
// 				require.Len(t, peerMq.WriteMsgCalls(), 1)
// 			}

...
func (v *BeefValidator) ValidateTransaction(ctx context.Context, beefTx *beef.BEEF, feeValidation validator.FeeValidation, scriptValidation validator.ScriptValidation) (*bt.Tx, error) {
	...
	for _, btx := range beefTx.Transactions {
		...
		if err := validation.CommonValidateTransaction(v.policy, tx); err != nil {
			return tx, err
		}
	...
	if feeValidation == validator.DeepFeeValidation {
		if err := deepCheckFees(beefTx, feeQuote); err != nil {
			return beefTx.GetLatestTx(), err
		}
	}

	if err := ensureAncestorsArePresentInBump(beefTx.GetLatestTx(), beefTx); err != nil {
		return beefTx.GetLatestTx(), validation.NewError(err, api.ErrStatusMinedAncestorsNotFound)
	}
...
func standardCheckFees(tx *bt.Tx, beefTx *beef.BEEF, feeQuote *bt.FeeQuote) error {
	expectedFees, err := validation.CalculateMiningFeesRequired(tx.SizeWithTypes(), feeQuote)
	if err != nil {
...
func deepCheckFees(beefTx *beef.BEEF, feeQuote *bt.FeeQuote) error {
...
func validateScripts(tx *bt.Tx, inputTxs []*beef.TxData) error {
	for i, input := range tx.Inputs {
		inputParentTx := findParentForInput(input, inputTxs)
		if inputParentTx == nil {
			return validation.NewError(errors.New("invalid parent transactions, no matching trasactions for input"), api.ErrStatusUnlockingScripts)
		}

		err := checkScripts(tx, inputParentTx.Transaction, i)
		if err != nil {
			return validation.NewError(errors.New("invalid script"), api.ErrStatusUnlockingScripts)
		}
	}

...
func TestValidator(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			se := goscript.NewScriptEngine("main")
			sut := New(policy, tc.finder, se, WithStandardFormatSupported(tc.stdFormatSupported), WithChronicleForkBlock(882687))
	...
	t.Run("valid tx multi", func(t *testing.T) {
		...
		for txIndex, txStr := range txs {
			...
			sut := New(policy, nil, se, WithChronicleForkBlock(15000))

			// when
			actualError := sut.ValidateTransaction(context.TODO(), tx, validator.StandardFeeValidation, validator.StandardScriptValidation, 15000)
...
func TestPerformStandardScriptValidation_BlockHeightZero(t *testing.T) {
	// given
	...
	// when — block height 0 should be rejected
	actualError := sut.ValidateTransaction(context.TODO(), tx, validator.NoneFeeValidation, validator.StandardScriptValidation, 0)

	// then
	require.Error(t, actualError)
	require.ErrorContains(t, actualError, "block height not yet available")
}
...
func BenchmarkValidator(b *testing.B) {
	...
	for i := 0; i < b.N; i++ {
		_ = sut.ValidateTransaction(context.TODO(), tx, validator.StandardFeeValidation, validator.StandardScriptValidation, 15000)
	}
}
...
func TestFeeCalculation(t *testing.T) {
	...
	// when
	err = sut.ValidateTransaction(context.TODO(), tx, validator.StandardFeeValidation, validator.StandardScriptValidation, 15000)

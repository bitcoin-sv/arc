...
import (
	...
	"github.com/bitcoin-sv/arc/k8s_coordinator/mock"
...
func TestStart(t *testing.T) {
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			coordinator := k8s_coordinator.New(metamorphMock, k8sClientMock, "test-namespace", k8s_coordinator.WithTicker(ticker),
				k8s_coordinator.WithLogger(slog.New(tint.NewHandler(os.Stdout, &tint.Options{Level: slog.LevelInfo, TimeFormat: time.Kitchen}))),
			)

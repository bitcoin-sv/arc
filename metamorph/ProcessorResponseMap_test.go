...
func TestNewProcessorResponseMap(t *testing.T) {
	...
	t.Run("go routine", func(t *testing.T) {
		...
		assert.True(t, ok)
		assert.Equal(t, metamorph_api.Status_SENT_TO_NETWORK, item.status)
...
func TestProcessorResponseMap_Delete(t *testing.T) {
	t.Run("default", func(t *testing.T) {
		...
		require.True(t, ok2)
		assert.Equal(t, metamorph_api.Status_ANNOUNCED_TO_NETWORK, item2.status)
...
func TestProcessorResponseMap_Get(t *testing.T) {
	...
	t.Run("status returned", func(t *testing.T) {
		...
		for _, tt := range tests {
			...
			if tt.ok {
				assert.Equal(t, tt.status, item.status)
			}
		}
	})
}

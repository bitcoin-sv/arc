package minedtransactionshandler
...
func New(l utils.Logger) *MinedTransactionHandler {
	...
	OUT:
		for {
			...
			case s := <-h.newSubscriptions:
				...
				h.logger.Infof("New MinedTransactions subscription received (Total=%d).", len(h.subscribers))
				// go func() {

...
func (m ArcDefaultHandler) processTransactions(ctx context.Context, txsHex []byte, options *metamorph.TransactionOptions) (
	submittedTxs []*transaction.Transaction, successes []*api.TransactionResponse, fails []*api.ErrorFields, processingErr *api.ErrorFields,
) {
	...
	for len(txsHex) != 0 {
		...
		if hexFormat == validator.BeefHex {
			...
			transaction, bytesUsed, err := transaction.NewTransactionFromStream(txsHex)
			if err != nil {
...
func (m ArcDefaultHandler) validateEFTransaction(ctx context.Context, txValidator validator.DefaultValidator, transaction *transaction.Transaction, options *metamorph.TransactionOptions) *api.ErrorFields {
...
func (m ArcDefaultHandler) submitTransactions(ctx context.Context, txs []*transaction.Transaction, options *metamorph.TransactionOptions) ([]*metamorph.TransactionStatus, *api.ErrorFields) {
...
func (ArcDefaultHandler) handleError(_ context.Context, transaction *transaction.Transaction, submitErr error) (api.StatusCode, *api.ErrorFields) {
	if submitErr == nil {
...
func prepareSizingInfo(txs []*transaction.Transaction) [][]uint64 {
...
func getSizings(tx *transaction.Transaction) (uint64, uint64, uint64) {

...
func TestPublish(t *testing.T) {
	...
	tt := []struct {
		...
		topic         string
		opts          []Option
		testFunc      func(cl *Client, topic string, msg *test_api.TestMessage)
		subscribeFunc func(cl *Client, topic string, messageChan chan *test_api.TestMessage)
	}{
		{
			name:  "publish marshal - work queue policy",
			topic: "pub-topic-1",
			opts: []Option{
				WithStream("pub-topic-1", "pub-topic-1-stream", jetstream.WorkQueuePolicy, false),
				WithConsumer("pub-topic-1", "pub-topic-1-stream", "pub-topic-1-cons", true, jetstream.AckExplicitPolicy),
				WithFileStorage(),
			},
			testFunc: func(cl *Client, topic string, msg *test_api.TestMessage) {
		...
		},
		{
			...
			topic: "pub-topic-2",
			opts: []Option{
				WithStream("pub-topic-2", "pub-topic-2-stream", jetstream.WorkQueuePolicy, true),
				WithConsumer("pub-topic-2", "pub-topic-2-stream", "pub-topic-2-cons", false, jetstream.AckExplicitPolicy),
			},
			testFunc: func(cl *Client, topic string, msg *test_api.TestMessage) {
		...
		},
		{
			...
			topic: "pub-topic-3",
			opts: []Option{
				WithStream("pub-topic-3", "pub-topic-3-stream", jetstream.InterestPolicy, false),
				WithConsumer("pub-topic-3", "pub-topic-3-stream", "pub-topic-3-cons", false, jetstream.AckExplicitPolicy),
			},
			testFunc: func(cl *Client, topic string, msg *test_api.TestMessage) {
				err = cl.PublishMarshal(context.TODO(), topic, msg)
				require.NoError(t, err)
	...
	for _, tc := range tt {
		t.Run(tc.name, func(t *testing.T) {
			...
			require.NoError(t, err)
			mqClient, err := New(natsConnClient, logger, tc.opts...)
			require.NoError(t, err)
...
func TestSubscribe(t *testing.T) {
	tt := []struct {
		...
		topic    string
		opts     []Option
		testFunc func(cl *Client, topic string, messageChan chan *test_api.TestMessage)
	}{
		{
			name:  "subscribe - work queue policy",
			topic: "sub-topic-1",
			opts: []Option{
				WithStream("sub-topic-1", "sub-topic-1-stream", jetstream.WorkQueuePolicy, false),
				WithConsumer("sub-topic-1", "sub-topic-1-stream", "sub-topic-1-cons", false, jetstream.AckExplicitPolicy),
			},
			testFunc: func(cl *Client, topic string, messageChan chan *test_api.TestMessage) {
				err = cl.Consume(topic, func(bytes []byte) error {
					serialized := &test_api.TestMessage{}
		...
		},
		{
			...
			topic: "sub-topic-2",
			opts: []Option{
				WithStream("sub-topic-2", "sub-topic-2-stream", jetstream.InterestPolicy, false),
				WithConsumer("sub-topic-2", "sub-topic-2-stream", "sub-topic-2-cons", false, jetstream.AckExplicitPolicy),
			},
			testFunc: func(cl *Client, topic string, messageChan chan *test_api.TestMessage) {

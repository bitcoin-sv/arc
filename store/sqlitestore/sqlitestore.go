...
func New(dbLocation string) (store.Store, error) {
	if dbLocation == "" {
...
func (s *SqliteStore) Get(ctx context.Context, hash []byte) (*store.StoreData, error) {
	q := `SELECT
	   stored_at
		,announced_at
		,mined_at
...
func (s *SqliteStore) Set(ctx context.Context, hash []byte, value *store.StoreData) error {
...
func (s *SqliteStore) GetUnseen(ctx context.Context, callback func(s *store.StoreData)) error {
	q := `SELECT
	   stored_at
		,announced_at
		,mined_at

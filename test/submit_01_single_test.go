...
func TestSubmitMined(t *testing.T) {
	t.Run("submit mined tx + calculate merkle path", func(t *testing.T) {
		...
		// Since this transaction was unregistered, it will not have a merkle path pre-calculated, so blocktx will need
		...
		case status := <-callbackReceivedChan:
			...
			require.Equal(t, merklePath, status.MerklePath)

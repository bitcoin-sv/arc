...
func New(fees *arc.FeesResponse) validator.Validator {
...
func (v *DefaultValidator) ValidateTransaction(tx *bt.Tx) error { //nolint:funlen - mostly comments
	...
	for _, input := range tx.Inputs {
		if input.PreviousTxScript == nil || (input.PreviousTxSatoshis == 0 && !input.PreviousTxScript.IsData()) {
			return validator.NewError(fmt.Errorf("transaction is not in extended format"), arc.ErrStatusTxFormat)
		}
	}

	// 1) Neither lists of inputs or outputs are empty
	if len(tx.Inputs) == 0 || len(tx.Outputs) == 0 {
		return validator.NewError(fmt.Errorf("transaction has no inputs or outputs"), arc.ErrStatusInputs)
	}

	// 2) The transaction size in bytes is less than maxtxsizepolicy.
	if err := checkTxSize(txSize, v.fees); err != nil {
		return validator.NewError(err, arc.ErrStatusTxFormat)
	}
	...
	if err := checkInputs(tx); err != nil {
		return validator.NewError(err, arc.ErrStatusInputs)
	}
	...
	if err := checkOutputs(tx); err != nil {
		return validator.NewError(err, arc.ErrStatusOutputs)
	}
	...
	if txSize < 100 {
		return validator.NewError(fmt.Errorf("transaction size in bytes is less than 100 bytes"), arc.ErrStatusMalformed)
	}
	...
	if err := sigOpsCheck(tx, v.fees); err != nil {
		return validator.NewError(err, arc.ErrStatusMalformed)
	}
	...
	if err := checkFees(tx, arc.FeesToBtFeeQuote(v.fees.Fees)); err != nil {
		return validator.NewError(err, arc.ErrStatusFees)
	}

	// 12) The unlocking scripts for each input must validate against the corresponding output locking scripts
	if err := checkScripts(tx); err != nil {
		return validator.NewError(err, arc.ErrStatusUnlockingScripts)
	}
...
func checkTxSize(txSize int, policy *arc.FeesResponse) error {
	maxTxSizePolicy := 1
...
func checkOutputs(tx *bt.Tx) error {
	...
	for index, output := range tx.Outputs {
		...
		switch {
		case !isData && (output.Satoshis > MaxSatoshis || output.Satoshis < arc.DustThreshold):
			return validator.NewError(fmt.Errorf("transaction output %d satoshis is invalid", index), arc.ErrStatusOutputs)
		case isData && output.Satoshis != 0:
			return validator.NewError(fmt.Errorf("transaction output %d has non 0 value op return", index), arc.ErrStatusOutputs)
		}
		total += output.Satoshis
	}

	if total > MaxSatoshis {
		return validator.NewError(fmt.Errorf("transaction output total satoshis is too high"), arc.ErrStatusOutputs)
	}
...
func checkInputs(tx *bt.Tx) error {
	...
	for index, input := range tx.Inputs {
		if hex.EncodeToString(input.PreviousTxID()) == coinbaseTxID {
			return validator.NewError(fmt.Errorf("transaction input %d is a coinbase input", index), arc.ErrStatusInputs)
		}
		...
		*/
		if input.PreviousTxSatoshis > MaxSatoshis {
			return validator.NewError(fmt.Errorf("transaction input %d satoshis is too high", index), arc.ErrStatusInputs)
		}
		total += input.PreviousTxSatoshis
	}
	if total > MaxSatoshis {
		return validator.NewError(fmt.Errorf("transaction input total satoshis is too high"), arc.ErrStatusInputs)
	}
...
func sigOpsCheck(tx *bt.Tx, policy *arc.FeesResponse) error {
	maxSigOps := 1
	if maxSigOps == 0 {
